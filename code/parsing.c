...
static char* expand_environ_var(char* arg) {
	// Your code here
    	...
    	//Parte 1: Imprimir variables de entorno
	if (arg[0] == '$'){
		char *newString = arg + 1;
		char *env = getenv(newString);
        //fix para la segunda parte
        if (env == NULL) {
		    strcpy(arg, " ");
		    return arg;
        }
        char* _env = (char*) malloc(sizeof(char)*(strlen(env)+1));
        free(arg);
        strncpy(_env, env, strlen(env));
        _env[strlen(env)] = '\0'; //fin de string
        return _env;
	}
	return arg;
